...
export default function Sidebar({
  ...
  return (
    <>
      {/* Backdrop: only rendered (and only visible below md) while the
          sidebar is expanded, so on phones the sidebar behaves like a
          slide-in drawer over the chat instead of squeezing it into a
          sliver. Tapping it closes the drawer. On md+ screens this never
          shows, since there the sidebar pushes the layout as normal. */}
      {!collapsed && (
        <div
      ...
      <div
        className={`border-panelBorder flex flex-col shrink-0 overflow-hidden bg-bg
          transition-[width,transform] duration-300 ease-in-out
          ${
            collapsed
              ? "relative w-[64px] p-3 border-r"
              : "fixed inset-y-0 left-0 z-40 w-[260px] p-5 border-r md:relative md:z-auto"
          }`}
      >
        {collapsed ? (
